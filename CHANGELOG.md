...
- `UploadImageTool`'s size cap was enforced only after the full body was already in memory (a fully base64-decoded payload, or a fully downloaded URL response), defeating the point of a cap against a large/malicious payload. Base64 payloads are now rejected by their encoded length before `base64_decode()` runs; URL fetches now send a `HEAD` request first and reject an oversized `Content-Length` before the body is ever downloaded.
